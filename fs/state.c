    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                // Introduzi for
                for(int i=0;i<10;i++){
                    inode_table[inumber].data_block[i] = b;
                }
                ...
                // Introduzi for
                for(int i=0;i<10;i++){
                    inode_table[inumber].data_block[i] = -1;
                }
                inode_table[inumber].i_data_block = -1;
                inode_table[inumber].i_index = 0;
            }
...
int inode_delete(int inumber) {
    ...
    if (inode_table[inumber].i_size > 0) {
        ...
        // Introduzi for
        for(int i=0;i<10;i++){
            data_block_free(inode_table[inumber].data_block[i]);
...
int find_in_dir(int inumber, char const *sub_name) {
    ...
    //pthread_mutex_lock(&dir_entry[inumber].lock);
